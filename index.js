...
function logWeirdStuff() {
	...
	const timer = setInterval( () => {
		console.log(message, "background:" +  changeHue('#9b59b6', 3 * totalAmount) + "; color: #ecf0f1; padding: 20px; font-size:1.3em");
		if (totalAmount > 4000) {
...
class TextChunk extends Component {
    ...
    componentDidUpdate(props) {
        if (this.props.type == 'writer' && this.props.shouldWrite && !this.timerCreated) {
            this.createTimer()
        } 
    }
    ...
    createTimer() {
        ...
        const timer = setInterval(() => {
            ...
            if (this.props.type == "writer") {
                ...
                this.setState({
                    ...this.state,
                    textWritten: this.state.textWritten.slice(0, newIndex) + newChar + this.state.textWritten.slice(newIndex + 1),
    ...
    render() {
        if(this.state.hasFinished) {
            if(this.isLink) {
                return html`<pre style="${this.props.style}"> <a href="${this.props.text}">${this.props.text}</a> </pre>`
            } else {
                return html`<pre style="${this.props.style}">${this.props.text}</pre>`
            }
        } else {
            if(this.isLink) {
                return html`<pre style="${this.props.style}"> <a href="${this.props.text}">${this.state.textWritten}</a> </pre>`
            } else {
                return html`<pre style="${this.props.style}"> ${this.state.textWritten} </pre>`
            }
        }
    }
}
...
class FizzleText extends Component {
    ...
    createColorMapping(props) {
        this.colorMapping = props.textList.map((_, index) => changeHue(this.INITIAL_GREEN, index * 2.3))
    }

    render() {
        return html`
            ${this.props.textList.map((text, index) => html`
                <${TextChunk} onFinished=${this.onFinished} text=${text} type="fizzle" style="color:${this.colorMapping[index]}"/>
            `)}`
    }
}
...
class WriterText extends Component {
    ...
    onFinished(index) {
        let newWriteOnOffMapping =  Object.assign([], this.state.writeOnOffMapping, {[index]: false})
        if (index < this.state.writeOnOffMapping.length - 1) {
            newWriteOnOffMapping = Object.assign([], newWriteOnOffMapping, {[index + 1]: true})
        }
        this.setState({
            ...this.state,
...
class Life extends Component {
    constructor() {
        ...
        this.state = {
            board: new Array(30).fill(null).map(() => new Array(80).fill(null).map(() => Math.floor(this.biasRandom(0, 2, 2.9))))
        }
        ...
        setInterval(() => {
            ...
            for(var i = 0;i < newBoard.length;i++) {
                for(var j = 0;j < newBoard[i].length;j++) {
                    ...
                    let numNeighbors = newBoard[this.mod(i + 1, newBoard.length)][j] +  //right
                        newBoard[this.mod((i + 1), newBoard.length)][this.mod(j + 1 ,newBoard[i].length)] +  // bottom right
                        newBoard[i][this.mod(j + 1, newBoard[i].length)] +  // bottom
                        newBoard[this.mod(i - 1, newBoard.length)][this.mod(j + 1, newBoard[i].length)] +  // bottom left
                        newBoard[this.mod(i - 1, newBoard.length)][j] + // left 
                        newBoard[this.mod(i - 1, newBoard.length)][this.mod(j - 1, newBoard[i].length)] + // left-up
                        newBoard[i][this.mod(j - 1, newBoard[i].length)] // up
                        newBoard[this.mod(i + 1, newBoard.length)][this.mod(j - 1, newBoard[i].length)] // right up
                    if (newBoard[i][j] == 1) {
                        // alive
    ...
    render() {
        return html`<${FizzleText} onFinished=${this.onFinished} textList=${this.getStringRepr()}/>`
    }
}
class App extends Component {
    render() {
        return html`<div><${WriterText} textList=${textToShow}/> <div class="life"><${Life} /> </div> </div>`
    }
}
